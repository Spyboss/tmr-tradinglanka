...
router.get('/finance-company-sales/pdf', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    headers.forEach((h, i) => {
      ...
      doc.fillColor('#ffffff').text(h, x + 5, y + 6, {
    ...
    doc.fillColor('#333333').font('Helvetica-Bold').fontSize(bodyFontSize)
      .text(`Total (${bills.length} bills)`, tableStartX + 5, y + 6, { width: totalColWidth - 82 })
      .text(formatCurrency(totalAmount), tableStartX + totalColWidth - 73, y + 6, { width: 68, align: 'right' });
