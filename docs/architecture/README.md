...
| Billing | `backend/src/routes/billRoutes.ts` & controllers | CRUD operations, ownership enforcement, PDF generation, integration with inventory (auto-create on manual entry, close-sale enforces inventory assignment, edit form allows changing inventory). |
| Inventory | `backend/src/routes/inventoryRoutes.ts` | Item lifecycle, analytics summaries, inventory report analytics (monthly sales pace, stock penalty alerts with chassis numbers), PDF exports, bike-model catalogue linkage, auto-creation on bill submit. |
...
| Reports | `backend/src/routes/reportRoutes.ts` | Finance Company Sales report with paginated list endpoint and landscape A4 PDF generation (dynamic row heights, dealer branding, summary bar). |
| PDF Services | `backend/src/services/*.ts` | PDF generation for bills, quotations, proforma, inventory reports, warranty claims. Includes configurable footer attribution ("Generated by UHADEV | Custom Business Systems"). |
| Warranty Claims | `backend/src/routes/warrantyClaimRoutes.ts` & controllers | Full CRUD for service point warranty claims with auto-numbering (WAR-YYMMDD-XXX), bilingual (EN/SI) PDF generation with NotoSansSinhala font, bill prefill (customer/vehicle data + color from inventory notes), typed warranty parts dropdown with serial number tracking, form number uniqueness validation (partial unique index + live availability check + next-number suggestion). |
| Frontend Shell | `frontend/src/App.jsx` | Authenticated routing, Ant Design layout, navigation. |
| Feature Pages | `frontend/src/pages` | Dashboard, bills, inventory, quotations, warranty claims, admin, verification. |
