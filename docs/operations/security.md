...
- Audit events recorded in `UserActivity` for key actions (bill create/update/delete, profile changes, etc.). Bill update and delete operations include detailed metadata with old/new values, inventory release/claim status, and IP address/user agent for traceability.
- **PDF Attribution**: Generated PDFs include a footer attribution ("Generated by UHADEV | Custom Business Systems") which can be customized via `PDF_DOCUMENT_ATTRIBUTION` environment variable.
