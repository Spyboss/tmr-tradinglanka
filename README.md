...
- **Reports** – Finance Company Sales report with per-company filtering, date range selection, server-side search, client-side within-results filtering, pagination, and landscape A4 PDF export with dynamic row heights and dealer branding.
...
- **Dynamic PDF Layouts** – Proforma invoice and finance company sales report PDFs use dynamic row/field heights via `heightOfString`, automatically expanding boxes to fit long customer names, addresses, and other wrapped text without clipping.
...
- `/api/reports` – Finance Company Sales report (list + PDF).
